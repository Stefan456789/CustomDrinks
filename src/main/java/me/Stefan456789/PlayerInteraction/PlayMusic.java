...
public class PlayMusic implements Listener {
    ...
    public void playerInteractEvent(PlayerInteractEvent e) {
        if ((e.getAction().equals(Action.RIGHT_CLICK_BLOCK) || e.getAction().equals(Action.LEFT_CLICK_BLOCK)) && e.getClickedBlock().getBlockData().getMaterial().equals(Material.JUKEBOX)) {
            ...
            if (jukebox.isPlaying() && Objects.requireNonNull(jukebox.getRecord().getItemMeta()).getCustomModelData() != 0) {
            ...
            if (!jukebox.isPlaying() && Objects.requireNonNull(e.getPlayer().getItemInHand().getItemMeta()).getCustomModelData() != 0 && e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
