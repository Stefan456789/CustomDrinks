...
public class PlayMusic implements Listener {
    ...
    public void playerInteractEvent(PlayerInteractEvent e) {
        if ((e.getAction().equals(Action.RIGHT_CLICK_BLOCK) || e.getAction().equals(Action.LEFT_CLICK_BLOCK)) && e.getClickedBlock().getBlockData().getMaterial().equals(Material.JUKEBOX)) {
            ...
            if (jukebox.isPlaying() && jukebox.getRecord().hasItemMeta() && jukebox.getRecord().getItemMeta().hasCustomModelData() && jukebox.getRecord().getItemMeta().getCustomModelData() != 0) {
            ...
            if (!jukebox.isPlaying() && e.getPlayer().getItemInHand().hasItemMeta() && e.getPlayer().getItemInHand().getItemMeta().hasCustomModelData() && e.getPlayer().getItemInHand().getItemMeta().getCustomModelData() != 0 && e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
                ...
                e.getPlayer().getServer().getScheduler().runTaskLaterAsynchronously(main, new Runnable() {
                    @Override
                    public void run() {
